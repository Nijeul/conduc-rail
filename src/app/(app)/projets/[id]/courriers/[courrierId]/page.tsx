import { getCourrier } from '@/actions/courriers'
...
interface Props {
  params: { id: string; courrierId: string }
}

export default async function CourrierDetailPage({ params }: Props) {
  const [courrier, infos] = await Promise.all([
    getCourrier(params.id, params.courrierId),
    getInfosProjet(params.id),
  ])

  if (!courrier || !infos) notFound()
